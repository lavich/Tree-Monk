...
/**
 * Idempotent schema migrations shared by BOTH runtimes: the Electron main
 * process (better-sqlite3) and the browser build (WASM SQLite adapter). Every
 * step is additive or one-time-gated, so running it on any DB is always safe.
 */
